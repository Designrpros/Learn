...
export function WikiContentMonitor({ topic, initialContent }: WikiContentMonitorProps) {
    ...
    useEffect(() => {
        if (!initialContent || initialContent === "Autogenerated stub.") {
            // Auto-trigger if not already generating and haven't tried yet
            if (!isThisTopicActive && !activeJob && !hasAttemptedAuto) {
                ...
                }, 1000); // 1s delay for smooth UX transition
                return () => clearTimeout(timer);
            }
        }
    }, [initialContent, isThisTopicActive, activeJob, hasAttemptedAuto, topic.id, startJob]);
